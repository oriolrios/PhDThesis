...
function [x fval Invarians CostVal] = ConstOptSmartQFIRE_all(xy_real,fuelA,aspectA,slopeA,weight,dt,I,OptIndexCh,fId)
    Invarians = [];
    CostVal = [];
...
%Definitions
    ...
    lb  = [0.01, 0.1, 0.01, 0, 5000,-2*pi()]; %low domain value
    ub  = [1.2, 0.8, 0.4, 20, 10000,2*pi()]; % high domain value
    nonlcon= [];
%     subjects the minimization to the nonlinear inequalities c(x) or 
%     equalities ceq(x) defined in nonlcon. 
    f=@(v) cost2opt_all(v,xy_real{1},xy_real,fuelA,aspectA,slopeA,weight,dt,OptIndexCh,fId);
