...
%
% History:
% $ 10.06.16 Fix problem when initial-end point are within a double loop!
%
...
while N_xy < length(x)/4 % To set a threashold to avoid close loops in initial points
    ...
    %c = combnk(1:size(x,1),2);% MASSA LENT
...
% DEBUG    
%     flag=flag+1; fprintf('%d',flag)

% Eliminate initial point to overcome the initial loop
    x=x(1:end-1);
    y=y(1:end-1);
