...
%     criterion.
%     The miminum of the two angles is determined and if it is less than approximately 90 degrees then the newx and newy points are inserted into the
%     previous x and y array and the process is repeated until the conditions are satisfied and the curve is adequately discretized.
%     the following algorithm is necessary when the fire front is making its way around a boundary and the curve produces sharp zones
%
%     ADD DEGRID capabilities in else case!
%     TRESHOLDS in LINE 42 & 58!!!
function [newxy]=regridding(x,y)
%T= 1;% Regridding threshold (Richards 1)
T=pi/10; % the smaller T the smoother the front!
norm_min = 0.5; % [m] min distance between nodes TO DEGRID (whatever angular relation!
%norm_min = 0.001;
norm_max = 5;  % [m] max distance between nodes TO REGRID (whatever angular relation!
% Sensiblitat actual >pi/4
if numel(x)<3
    error('Not enought points to degrid')
end
...
newxy=[newx(1:end-1) newy(1:end-1)]; % eliminate the last one that is repeated
