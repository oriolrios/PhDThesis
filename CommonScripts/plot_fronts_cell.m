...
%
% PLOT_FRONTS_CELL(xy_fronts,'hold','[0 0.5 0.5]')
%
% INPUT
...
if length(xy_fronts)>60
    color_map=colormap(hsv(length(xy_fronts)));
else
...
if nargin>2
    ...
    for t=1:length(xy_fronts)
        %Hax=plot (xy_fronts{t}(:,1), xy_fronts{t}(:,2), '-', 'color', color_map(2*t,:));
        Hax=plot (xy_fronts{t}(:,1), xy_fronts{t}(:,2), '-', 'color', color_map(2*t,:));
