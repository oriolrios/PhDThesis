function isochrones = csvqgis2isochrones(datafile)
%
%
...
isochrones.xy = cell(size(expanding_fire, 1), 1);
isochrones.timesec = cell(size(expanding_fire, 1), 1);

% for i = 1 : size(coordinates_column, 1) % Isochrones loop
for i = 1 : size(expanding_fire, 1) % Isochrones loop
%     dumm_string = coordinates_column{i}; % Get the string in the first column
    dumm_string = expanding_fire{i, 'WKT'}; % Get the string in the first column
