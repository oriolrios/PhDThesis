...
% ----------%
OptWeightStrng='idty';  %{'idty','lin','linN','expN'};
...
%     [I,filename{i}]=SmartQFIRE_func(OptIndexStrng{i},ax1(i));
    tic
    [Inv,filename{i},xy_model{i},xy_real,xy_ini_guess,costVal(i),info_str]=SmartQFIRE_func_all(OptIndexStrng{i},OptWeightStrng); % TO avoid ploting index exploration
    runTime(i)=toc;
    CellI{i}=Inv;
    Invariants(i,:)=Inv(end,:);
