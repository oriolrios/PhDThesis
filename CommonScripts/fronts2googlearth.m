function fronts2googlearth(filename, FRONTS,zone,hemisphere,varargin)
%function grd2googleearth(FRONTS,bounds,zone,hemisphere,datum)
% INPUT
...
%
% HISTORY
% $24/01 -  LABELS min instead of SEC (decimal label if not round minutes)
%
narginchk(2, 6);

datum=70;
if ~isstruct(FRONTS) %if it's not an struct with INFO on time %%Assume time step 1min (60sec)
    fronts.xy = FRONTS;
    fronts.time =60*[0:1:size(fronts.xy,1)-1]'; 
    disp('Assiming time intervals every 60s')
else
    fronts=FRONTS;
end

fronts_times=length(fronts.time);
Style_id_fronts_times=cell(1,fronts_times);
StyleUrl_id_fronts_times=cell(1,fronts_times);
...
if nargin>=5 && ischar(varargin{1})
    ...
    switch color
        case {'k','black'}
            fronts_colormap=zeros(fronts_times,3);
        case {'b', 'blue'}
            fronts_colormap=zeros(fronts_times,3);
...
if fronts_times>kk
    k=fronts_times-kk;
    fronts_colormap(kk+1:fronts_times,:)=fronts_colormap(1:k,:);
end
% ADD mask info
...
%Folder_name=
%  &lt;sub&gt;0&lt;sub&gt; <sub>0<sub>
for i=1:fronts_times
    %Style_id_fronts_times{i}=upper(sprintf('t &lt;sub&gt;0&lt;sub&gt; + %d min',fronts.time(i)));
    
    % time in SEC
%     Style_id_fronts_times{i}   =upper(sprintf('t0 + %d sec',fronts.time(i)));
%     StyleUrl_id_fronts_times{i}=upper(sprintf('t0 + %d sec',fronts.time(i)));
    % time in min
    Style_id_fronts_times{i}   =upper(sprintf('t0 + %.2g min',fronts.time(i)/60)); %perfecte la %g!
    StyleUrl_id_fronts_times{i}=upper(sprintf('t0 + %.2g min',fronts.time(i)/60));
...
for i=1:fronts_times
    ...
    [lat,long]=utm2latlon(fronts.xy{i}(:,1),fronts.xy{i}(:,2),zone,hemisphere,datum);
