function [AllSpeedMapStrucGRD,AllDirMapStrucGRD]=LoadAllBaseMaps(DirMapFolder)
...
for i=1:length(Deg)
    for j=1:length(AllU)
        [AllSpeedMapStrucGRD.dataStrc{i,j},AllDirMapStrucGRD.dataStrc{i,j}]=getCase(AllU(j),Deg(i),VegType{1},DirMapFolder,map_name,WNoutResolutionStr);
    end
end
...
%         MagMap=flipud(MagMap(:,1:end-1)); %correction due to file format!----> això està molt malament!
%         
%         
%         DirMap=dlmread(filenameAng,'\t',6,0);%DEG
%         DirMap=flipud(DirMap(:,1:end-1)); %correction due to file format!----> això està molt malament!
%          
%         cd(o_path)
%     end
