function [area_XOR,SDI,SORENSEN,JACCARD,SDI_a,SORENSEN_a,JACCARD_a,Bianchi]=area_between_curves_asBianchi(xy_front_real,xy_front_model,varargin)
%% [cost_area]=area_between_curves(xy_front_real,xy_front_model,varargin)
% Funtion called and controlled by: real_model_AREA_cost()
...
%         initial_area= polyarea(xy_front_real{1,1}(:,1),xy_front_real{1,1}(:,2));
    end
...
%   My_SDI(i,1)=cost_area(i,1)/real_area*100; rejected paper
    ...
    %% BOANCHI
    Bianchi =((area_AND-area_XOR)-initial_area)/(area_AND-initial_area);% as Bianchi 2010;
    %Bianchi =1-((area_AND-area_XOR)-initial_area)/(area_AND-initial_area);% as Bianchi 2010-> INVERTED
