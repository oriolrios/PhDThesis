function [AllSpeedMapStrucGRD,AllDirMapStrucGRD]=LoadAllBaseMapsC(DirMapFolder)
%
% C SUPPORTED VERSION (reads less fields!)
%
...
% $27.10.16 Only reads 1 vegetation. The first!!
%% PROGRAMM
...
[MagMapGRD]=getUCaseGRDStruct(DirMapFolder); %Uses MEX
...
AllSpeedMapStrucGRD.dataStrc = ones(MagMapGRD.dims(1),MagMapGRD.dims(2),length(Deg),length(AllU));
AllDirMapStrucGRD.dataStrc   = ones(MagMapGRD.dims(1),MagMapGRD.dims(2),length(Deg),length(AllU));
...
% fprintf('All maps loaded. Memory=%.2f MB\n',Vsize)
end

%% STANDALONE (ex NESTED FUNCTIONS)
