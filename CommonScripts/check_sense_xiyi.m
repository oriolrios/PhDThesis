...
[geom,n1,n2]=polygeom( xiyi(:,1), xiyi(:,2)); % ~ no ADIMAT
...
if v1_X_v2(3)>0     % counter clockwise!
    if rev_flag==1
        ...
        xiyi_ok=xiyi;
        inverted_flag=0;
        disp('xiyi is already counter-clockwise (as should!). Not inverted')
    end
elseif v1_X_v2(3)<0 % clockwise!
    if rev_flag==1
        xiyi_ok=xiyi;
        inverted_flag=0;
        disp('xiyi is already clockwise. Not inverted (reverse ON)')
        %error('Stophere')
    else
        xiyi_ok=flipud(xiyi);
