function [xyout]=huygns_integration_s_all(x,y,Mf,Mx,SAV,Wo,U,wind_dir,D,aspect,slope,At)
...
%     y=xy(:,2);
    ...
    fuel_depth=aspect;
    fuel_depth.data=D.*ones(size(fuel_depth.data));
...
%     [wind_dir_i]    =0.*ones(length(x),1);
  ...
  % NEW all variables
    [RoS_i,theta_w_s_i]=RothSlopeSI(U, wind_dir_i,fuel_depth_i, slope_i, aspect_i, SAV, Wo, Mf, Mx);
...
%     RoS_i=0.5.*ones(size(RoS_i));% m/s (30m/min)
    ...
    [x,y]=do_expansion(x,y,a,b,c,theta_w_s_i,aspect_i,slope_i,dt,resolution);
end
